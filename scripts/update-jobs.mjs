...
async function getOfferJobs(key) {
  if (!Number.isInteger(maxPages) || maxPages < 1 || maxPages > 50) throw new Error('MAX_PAGES must be 1–50');
  const items = [];
  let cursor;
...
async function main() {
  ...
  console.log(`已生成 ${jobs.length} 条记录；来源：${[permissionOffer && 'offer先生', permissionXixicc && 'xixicc2027'].filter(Boolean).join('、')}`);
}
